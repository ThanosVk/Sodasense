...
class SqlDatabase {
  ...
  // New function to fetch step data for the last week
  Future<List<Map<String, dynamic>>> fetchWeeklyStepData() async {
    final db = await instance.database;

    // Calculate the start of the current week
    final DateTime now = DateTime.now();
    final int weekday = now.weekday;
    // Assuming the week starts on Sunday (use weekday - 1 for Monday)
    final DateTime startOfWeek = now.subtract(Duration(days: weekday));

    final int startOfWeekMillis = DateTime(
        startOfWeek.year,
        startOfWeek.month,
        startOfWeek.day
    ).millisecondsSinceEpoch;

    // Query to select steps data from the start of the current week
    final result = await db.rawQuery(
        'SELECT date, steps FROM daily_steps WHERE date >= $startOfWeekMillis ORDER BY date ASC'
    );

    return result;
  }

  // Method to fetch today's steps
  Future<int> fetchTodaySteps() async {
    final db = await instance.database;
    final todayStart = DateTime.now();
    final todayStartMillis = DateTime(todayStart.year, todayStart.month, todayStart.day).millisecondsSinceEpoch;

    var result = await db.rawQuery(
        'SELECT SUM(steps) as todaySteps FROM daily_steps WHERE date >= $todayStartMillis');

    // Ensuring that the result is not null and is an integer
    if (result.isNotEmpty && result.first['todaySteps'] != null) {
      return int.tryParse(result.first['todaySteps'].toString()) ?? 0;
    }
    return 0;
  }

  Future<int> fetchMaxStepsInWeek() async {
    final db = await instance.database;
    ...
    final int weekday = now.weekday;
    final DateTime startOfWeek = now.subtract(Duration(days: weekday));
    final int startOfWeekMillis = DateTime(startOfWeek.year, startOfWeek.month, startOfWeek.day).millisecondsSinceEpoch;

    // Query to select the maximum steps in the past week
    var result = await db.rawQuery(
        'SELECT MAX(steps) as maxSteps FROM daily_steps WHERE date >= $startOfWeekMillis'
    );

    if (result.isNotEmpty && result.first['maxSteps'] != null) {
      return int.tryParse(result.first['maxSteps'].toString()) ?? 0;
    }
    return 0;
  }

  // New function to update steps in real-time (to be called whenever steps are updated)
  Future<void> updateStepData(int date, int steps) async {
    final db = await instance.database;

    // Update steps for the given date
    await db.rawUpdate(
        'UPDATE daily_steps SET steps = ? WHERE date = ?',
        [steps, date]
    );
  }
